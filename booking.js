...
function showSuccessModal(booking, event) {
    ...
    if (modal && successMessage && ticketDetails) {
            ...
            ticketBody.innerHTML = `
                ...
                <div class="ticket-info-row">
                    <div>
                        ...
                        <div class="ticket-value">${formatDate(event.date)} at ${event.time}</div>
                    </div>
                </div>
                <div class="ticket-info-row">
                    <div>
                        <div class="ticket-label">Location</div>
                        <div class="ticket-value">${event.location}</div>
                    </div>
                </div>
                <div class="ticket-info-row">
                    <div>
                        <div class="ticket-label">Attendee</div>
                        <div class="ticket-value">${booking.fullname}</div>
                    </div>
                </div>
                <div class="ticket-info-row">
                    <div>
                        <div class="ticket-label">Quantity</div>
                        <div class="ticket-value">${booking.quantity} Ticket(s)</div>
                    </div>
                </div>
                <div class="ticket-info-row">
                    <div>
                        <div class="ticket-label">Total Amount</div>
                        <div class="ticket-value">${formatCurrency(booking.totalPrice)}</div>
                    </div>
                </div>
            `;
        }
        ...
        if (ticketQR) {
            ticketQR.innerHTML = `
                <div style="text-align: center; font-size: 0.75rem; color: var(--deep-navy); width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                    <div style="font-size: 2.5rem; margin-bottom: 0.25rem; font-family: monospace; letter-spacing: 2px;">▦▦▦</div>
                    <div style="font-weight: 600; margin-bottom: 0.25rem;">QR CODE</div>
                    <div style="font-size: 0.6rem; opacity: 0.7;">${bookingId}</div>
                </div>
            `;
        }
